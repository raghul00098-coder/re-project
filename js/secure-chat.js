...
// --- STANDARD WEB CRYPTO END-TO-END RECIPIENT UTILITIES WITH IFRAME FALLBACKS ---
const isCryptoSupported = !!(window.crypto && window.crypto.subtle && window.crypto.getRandomValues);

if (!isCryptoSupported) {
  console.warn("CyberShield Security Center: window.crypto.subtle is unavailable in this browser environment. Engaging sandboxed software-based cryptographic fallback simulator.");
}
...
const SecureCrypto = {
  ...
  async generateKeyPair() {
    if (!isCryptoSupported) {
      return {
        publicKey: { type: "public", id: "mock-public-key-" + Math.random().toString(36).substr(2, 9) },
        privateKey: { type: "private", id: "mock-private-key-" + Math.random().toString(36).substr(2, 9) }
      };
    }
  ...
  async exportKeyJwk(key) {
    if (!isCryptoSupported) {
      return { kty: "RSA", n: "mock-modulus-" + key.id, e: "AQAB" };
    }
  ...
  async importKeyJwk(jwk, usages) {
    if (!isCryptoSupported) {
      return { type: usages.includes("encrypt") ? "public" : "private", id: jwk.n || "imported-key" };
    }
  ...
  async generateAesKey() {
    if (!isCryptoSupported) {
      return { type: "aes", id: "mock-aes-key-" + Math.random().toString(36).substr(2, 9) };
    }
    return await window.crypto.subtle.generateKey(
      {
        name: "AES-GCM",
  ...
  async exportAesKeyRaw(key) {
    if (!isCryptoSupported) {
      return new TextEncoder().encode(key.id).buffer;
    }
  ...
  async importAesKeyRaw(rawBuffer) {
    if (!isCryptoSupported) {
      const decodedId = new TextDecoder().decode(rawBuffer);
      return { type: "aes", id: decodedId };
    }
    return await window.crypto.subtle.importKey(
      "raw",
      rawBuffer,
  ...
  async encryptAesKey(aesKey, rsaPublicKey) {
    if (!isCryptoSupported) {
      return btoa("MOCK_RSA_ENC:" + aesKey.id);
    }
  ...
  async decryptAesKey(encryptedAesKeyBase64, rsaPrivateKey) {
    if (!isCryptoSupported) {
      const rawStr = atob(encryptedAesKeyBase64);
      const aesId = rawStr.startsWith("MOCK_RSA_ENC:") ? rawStr.substring(13) : "fallback-key";
      return { type: "aes", id: aesId };
    }
  ...
  async encryptPayload(text, aesKey) {
    if (!isCryptoSupported) {
      ...
      return {
        ciphertext: btoa("MOCK_AES_ENC:" + text),
  ...
  async decryptPayload(ciphertextBase64, ivBase64, aesKey) {
    try {
      if (!isCryptoSupported) {
        const rawStr = atob(ciphertextBase64);
        if (rawStr.startsWith("MOCK_AES_ENC:")) {
          return rawStr.substring(13);
        }
        return rawStr;
      }
