...
const initDashboard = () => {
  ...
  function monitorActiveSession() {
    let displayName = 'Agent Alice';
    let displayCallsign = 'AGENT_ALICE';
    ...
    if (welcomeTitle) {
      welcomeTitle.textContent = `CONSOLE LINK ACTIVE: WELCOME ${displayName.toUpperCase()} TO SECURITY OPERATIONS CENTER`;
    }
  ...
  async function handleLogout() {
    ...
    // Clear operator profile from localStorage as well
...
Date & Time: ${timestamp}`;
  ...
  // --- 24. UNIFIED CYBERSHIELD GLOBAL LAYOUT ENGINE ---
  function initGlobalLayoutEngine() {
    const updateMetrics = () => {
      const header = document.querySelector('.top-navbar') || document.getElementById('top-navbar');
      const bottomNav = document.querySelector('.bottom-nav-bar') || document.getElementById('bottom-nav-bar');
    ...
    // Interval check to keep it fully aligned dynamically
    setInterval(updateMetrics, 500);
  }

  initGlobalLayoutEngine();
