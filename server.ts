...
import { pageVerifyJWT, JWT_SECRET } from "./src/middleware/auth.ts";
...
// Premium High-Contrast Fallback Cyber Security Intel Bulletins
const FALLBACK_ARTICLES = [
  {
    title: "Critical Zero-Day in OpenSSL Allows Remote Code Execution",
    description: "A newly discovered vulnerability in OpenSSL versions 3.0.x allows unauthenticated remote attackers to execute arbitrary code on vulnerable servers. Security teams are urged to apply patches immediately.",
    image: "https://images.unsplash.com/photo-1614064641938-3bbee52942c7?auto=format&fit=crop&w=600&q=80",
    source: "The Hacker News",
    category: "Zero-Day",
    publishedDate: "Today, 10:15",
    pubDate: "Today, 10:15",
    pubDateTimestamp: Date.now(),
    articleUrl: "https://thehackernews.com",
    link: "https://thehackernews.com",
    author: "Threat Intel Unit",
    videoUrl: null,
    riskLevel: "CRITICAL"
  },
  {
    title: "Ransomware Group Targets Global Healthcare Provider with Triple Extortion",
    description: "An advanced persistent threat (APT) group has deployed asymmetric ransomware across a major healthcare network, demanding $15M and threatening to leak sensitive patient health records.",
    image: "https://images.unsplash.com/photo-1601597111158-2fceff270190?auto=format&fit=crop&w=600&q=80",
    source: "BleepingComputer",
    category: "Ransomware",
    publishedDate: "Today, 08:30",
    pubDate: "Today, 08:30",
    pubDateTimestamp: Date.now() - 3600000,
    articleUrl: "https://www.bleepingcomputer.com",
  ...
  {
    title: "New Phishing Campaign Evades Modern SEGs Using SVG Image Smuggling",
    description: "Security researchers have identified a sophisticated phishing campaign utilizing SVG files to smuggle malicious JavaScript payloads, completely bypassing Secure Email Gateways (SEGs).",
    image: "https://images.unsplash.com/photo-1526374965328-7f61d4dc18c5?auto=format&fit=crop&w=600&q=80",
    source: "SecurityWeek",
    category: "Phishing",
    publishedDate: "Yesterday, 14:22",
    pubDate: "Yesterday, 14:22",
    pubDateTimestamp: Date.now() - 86400000,
    articleUrl: "https://www.securityweek.com",
  ...
  {
    title: "Government Warns of Active Exploitation of Windows Kernel Vulnerability",
    description: "CISA has added a privilege escalation vulnerability in the Windows Common Log File System (CLFS) driver to its Known Exploited Vulnerabilities catalog, citing active targeting by state-sponsored actors.",
    image: "https://images.unsplash.com/photo-1504384308090-c894fdcc538d?auto=format&fit=crop&w=600&q=80",
    source: "CISA",
    category: "Government",
    publishedDate: "Yesterday, 11:05",
    pubDate: "Yesterday, 11:05",
    pubDateTimestamp: Date.now() - 90000000,
    articleUrl: "https://www.cisa.gov",
  ...
  {
    title: "Data Breach Exposes 2.4 Million Customer Profiles of E-Commerce Giant",
    description: "A misconfigured Amazon S3 bucket has exposed a database containing names, email addresses, hashed passwords, and purchase histories of over 2.4 million e-commerce customers.",
    image: "https://images.unsplash.com/photo-1563986768609-322da13575f3?auto=format&fit=crop&w=600&q=80",
    source: "Krebs on Security",
    category: "Data Breach",
    publishedDate: "Yesterday, 09:15",
    pubDate: "Yesterday, 09:15",
    pubDateTimestamp: Date.now() - 100000000,
    articleUrl: "https://krebsonsecurity.com",
  ...
  {
    title: "Adversary-in-the-Middle (AiTM) Phishing Kits Saw 300% Growth in 2025",
    description: "Reports show a massive surge in the adoption of AiTM phishing kits that can bypass classic Multi-Factor Authentication by capturing session cookies in real-time during proxying.",
    image: "https://images.unsplash.com/photo-1526374965328-7f61d4dc18c5?auto=format&fit=crop&w=600&q=80",
    source: "SecurityWeek",
    category: "Phishing",
    publishedDate: "2 days ago",
    pubDate: "2 days ago",
    pubDateTimestamp: Date.now() - 172800000,
    articleUrl: "https://www.securityweek.com",
    link: "https://www.securityweek.com",
    author: "SecurityWeek",
    videoUrl: null,
...
async function refreshCyberNewsCache() {
  ...
  if (allArticles.length > 0) {
    ...
    console.warn("Cyber News refresh fetched 0 new articles. Checking if cache is empty...");
    if (newsCache.length === 0) {
      console.log("Populating Cyber News Cache with premium static security alerts fallback.");
      newsCache = FALLBACK_ARTICLES;
      lastFetchTime = Date.now();
    }
  }
...
// Setup CORS - Permissive for Dev & AI Studio frame environments
app.use(cors({
  origin: (origin, callback) => {
    // Return true for all origins to ensure sandboxed iframes can access endpoints smoothly
    callback(null, true);
  },
  credentials: true
}));
...
// Simple Local JSON-file-based User Database
interface User {
  username: string;
  email: string;
  passwordHash: string;
...
function loadUsers(): Record<string, User> {
  try {
    ...
    console.error("Failed to load users database, falling back to memory:", err);
  }
  
  // Seed default operator user if file doesn't exist or failed to load
  const defaultHash = bcrypt.hashSync("password", 10);
  const initialUsers: Record<string, User> = {
    "operator@cybershield.com": {
      username: "operator",
...
function saveUsers(users: Record<string, User>) {
  try {
    ...
    console.error("Failed to persist users database:", err);
  }
}

// Initialize users database on server startup
...
app.post("/api/auth/register", (req, res) => {
  try {
    ...
    if (usersDb[normalizedEmail]) {
      return res.status(409).json({ error: "Email is already registered under a clearance level." });
    }
...
app.post("/api/auth/login", (req, res) => {
  try {
    ...
    if (!user || !bcrypt.compareSync(password, user.passwordHash)) {
      return res.status(401).json({ error: "Invalid credentials or unauthorized clearance ID." });
    }
...
app.post("/api/auth/logout", (req, res) => {
  ...
  res.json({ success: true, message: "Logged out successfully. Cryptographic session closed." });
});
